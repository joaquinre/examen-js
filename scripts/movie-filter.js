...
const rate = document.getElementById('user-rate')


function filterMovies(USERS, MOVIES, userId, fromDate, toDate, rate) {
  const _userId = parseInt(userId.value);
  const _rate = rate.value ? parseInt(rate.value) : null;
    ...
    return `
        <div class="card">
          ...
          <div class=" card-description">
            <h4>${result.movie}</h4>
            <p>${result.date}</p>
            <h6><span>Rate</span>${result.rate}</h6>
            <h6><span>Id</span>${result.userId}</h6>
          </div>
        </div>
      `;
  }).join('')
